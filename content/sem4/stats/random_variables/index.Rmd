...
Suppose $X ~ B(n, p)$ (binomial dist). We know:
...
$$
z = \frac{x - \mu}{\sigma} = \frac{x - np}{\sqrt{np (1 - p)}} ~ N(0, 1)
$$
...
Notation: $X ~ \text{Gamma}(\alpha , \beta )$, $\alpha$ is the shape parameter, and $\beta$ is the scale parameter.

PDF:

$$
f(x) \int \frac{x^{\alpha - 1} e^{-\frac{x}{\beta}}}{\beta^{2} T( \alpha )}
$$
...
$$
T( \alpha ) = \int_{0}^{\infty} x^{\alpha - 1} e^{-x} dx
$$

If $n$ is an integer, $T(n) = (n - 1)!$.
...
$Y$ is a chi-square RV with **degrees of freedom** $V = s \alpha$.
...
$$
f(y) = \int \frac{y^{\frac{V}{2} - 1} e^{- \frac{y}{2}}}{2^{\frac{V}{2}} T(\frac{V}{2})}
$$
...
* $Y$ is an exponential RV, $Y ~ \text{Exp}( \beta )$, $\beta > 0$.
