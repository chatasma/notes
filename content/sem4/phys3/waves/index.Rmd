...
Recall, `+` in this context means the distance becomes greater between the source and the destination, so the tuning fork that is getting further away from the person would have a `+` sign for its Doppler effect expression; conversely, the tuning fork that is approaching the person would have a `-` sign. Now, it is simple to calculate the frequencies, but let us first look at the beats frequency expression
