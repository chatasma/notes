...
* Want to maximimize distance between clusters, but minimize distance within clusters.
  * Within Cluster SS is the smu of squares of each point of a cluster to its centroid
...
![Centrality Measures](../../../../img/agglomerative_clustering.png)
...
See HW9 (Chapter 5 Exercise 9a) for an example of a **itemset lattice**, a hierarchical-like diagram which begins with the empty set $k = 0$, and branches to $k + 1$ if its parents are all frequent itemsets. Children which are omitted are considered "pruned".
...
* If an itemset is infrequent, its supersets are infrequent (the optimization step, the pruning in the itemset lattice).
...
* **Cosine similarity**: Where $x$ is the user profile and $i$ is the item profile:
  ...
  $$
    * 1 mean similar, 0 or -1 means not simila (in the unit circle, cosine is 1 when angle is small).
